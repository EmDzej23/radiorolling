...
public class SqlBuilder {
	...
	public SqlBuilder wheres(String[] id, String[] params) {
		this.where = "WHERE ";
		String and = "";
		for (int i = 0; i < id.length; i++) {
			and = i > 0 ? "AND" : "";
			this.where += and+" t." + id[i] + " = (:" + (params[i]) + ")";
		}
	...
	public <T> String insertQuery(Class<T> clazz, String json) {
		...
		String query = "Insert into " + clazz.getSimpleName().toLowerCase() + " (" + getKeys(json) + ") VALUES ("
				+ questionMarks + ")";
		return query;
	}
